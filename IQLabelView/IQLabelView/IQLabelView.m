...
- (void)rotateViewPanGesture:(UIPanGestureRecognizer *)recognizer
{
    ...
    if ([recognizer state] == UIGestureRecognizerStateBegan) {
        ...
        if (scaleRect.size.width >= (1+globalInset*2) && scaleRect.size.height >= (2+(globalInset*2)+2)) {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if([delegate respondsToSelector:@selector(shouldChangeCharactersInRange:)]) {
        [delegate shouldChangeCharactersInRange:self];
    }
    //[textField adjustsWidthToFillItsContents];
    return YES;
}
